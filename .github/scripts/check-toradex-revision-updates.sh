...
current_tag=$(grep -oP 'TORADEX_BRANCH:\s*\K.*' $SCRIPT_DIR/../workflows/yocto-toradex-optologic-ci.yml) || {
...
if [ "$latest_stable_tag" != "$current_tag" ]; then
    ...
    sed -i "s/TORADEX_BRANCH:\s*$current_tag/TORADEX_BRANCH: $latest_stable_tag/g" $SCRIPT_DIR/../workflows/yocto-toradex-optologic-ci.yml || {
        echo -e "Failed to update the yocto-toradex-optologic-ci.yml workflow file with the new Toradex tag.\n"
        exit 1
    }

    sed -i "s/TORADEX_BRANCH:\s*$current_tag/TORADEX_BRANCH: $latest_stable_tag/g" $SCRIPT_DIR/../workflows/torizon-optologic-binary.yml || {
